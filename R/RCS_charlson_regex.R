...
prCharlson_Armitage2010_regex <- 
  ...
  acute_icd_codes <- c(icd10= '^(I2[123]|J46|N17[12]|N19)',
  ...
  out <- prGetOutVector(out, charlson_4_comparing)
  
  # Do the actual test loop
  out <- prCheckCodes(out = out, 
                      icdCode = icdCode, 
                      score_codes = charlson_4_comparing,
                      code_is_icd10 =  code_is_icd10,
                      include_acute = include_acute,
                      acute_identifier_regex = acute_icd_codes[sprintf("icd%d", icd_ver)])
